...
def plotResults(files, algos, num_algs):
    ...
    for i in range(num_algs):
        ...
        fig.savefig(files[num_algs-1] + '/n_drop_avg_accu.png', bbox_inches='tight')
    fig = plt.figure()
    ax2 = fig.add_subplot(1, 1, 1)
    for i in range(num_algs):
        metric = 'loss'
        plotOneAlg(files[i],metric,marker[i],labels[i],ax2)
        fig.savefig(files[num_algs-1] + '/n_drop_avg_loss.png', bbox_inches='tight')


if __name__ == '__main__':
    lines = 0

    df = pd.read_csv("results/n_drop/name_file_res_algos.csv")

...
def plotResults(file_name_fedavg, file_name_fedprox, file_name_fedbn, file_name_fedap, file_name_metafed):
    ...
    ax1.plot(n_rounds, test_acc_fedavg_nbClient, label='FedAvg')
    ax1.plot(n_rounds, test_acc_fedprox_nbClient, label='FedProx')
    ax1.plot(n_rounds, test_acc_fedben_nbClient, label='FedBen')
    ax1.plot(n_rounds, test_acc_fedap_nbClient, label='Fedap')
    ax1.plot(n_rounds, test_acc_metafed_nbClient, label='Metafed')
    plt.legend()
    ...
    fig.savefig('N_Rounds_Accu.png', bbox_inches='tight')
